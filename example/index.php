 ...
 *
 */
$soundcloud = $client->getConnection('soundcloud');
if ($soundcloud) {
    if (!$soundcloud->isOAuth()) {
        $authSoundCloud = $soundcloud->getOAuth();
        $authSoundCloud->setRedirectUri('http://localhost:8001/');
        if ($authSoundCloud->isGetCode() !== true) {
            printf(
                '<a href="%s"> Login SoundCloud </a> <br>',
                $authSoundCloud->getAuthorizationUrl()
            );
        } else {
            $soundcloud->registerOAuth();
        }
    } else {
        printf('Logado no SoundCloud como %s <br>', $soundcloud->owner['name']);
    }

    if (!empty($soundcloud->owner)) {
        array_push($connections, $soundcloud->owner);
    }
}
